...
- [Публикация провайдера в private terraform registry](#публикация-провайдера-в-private-terraform-registry)
    ...
    - [Загрузка новой версии](#загрузка-новой-версии)
        - [Ручной запуск скрипта](#ручной-запуск-скрипта)
        - [Запуск скрипта средствами автоматизации](#запуск-скрипта-средствами-автоматизации)
...
- [Выпуск внутренней документации](#выпуск-внутренней-документации)
    - [Ручная публикация документации](#ручная-публикация-документации)
    - [Автоматическая публикация документации](#автоматическая-публикация-документации)
...
Так же unit тесты запустятся в PR'е с помощью автоматизации.

**Опционально.** Если PR связан с изменениями в `aws-sdk-go`, то можно сделать так, чтобы изменения из PR'а в `aws-sdk-go` так же подтянулись для прогона тестов, для этого нужно выполнить одно из двух условий:

- иметь одинаковые имена веток для провайдера и `aws-sdk-go`
- указать в описании PR'а в провайдере следующую конструкцию:

```
...
aws-sdk-go=<имя ветки в aws-sdk-go>
...
```
...
### Ручная публикация документации
